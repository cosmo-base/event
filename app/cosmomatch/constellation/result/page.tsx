...
import { Award, RefreshCw, BookOpen, Star, Loader2, ChevronRight } from "lucide-react"
...
function ResultContent() {
  ...
  useEffect(() => {
    getConstellations().then((data) => { setConstellations(data); setIsLoaded(true) }).catch(() => setIsLoaded(true))
  }, [])

  const userScores = useMemo(() => {
    const scores = {} as Record<keyof ConstellationStats, number>
    STAT_KEYS.forEach((key, index) => { scores[key] = parseInt(encodedStats[index] || "3", 36) })
    return scores
  }, [encodedStats])
  ...
  const totalDiff = STAT_KEYS.reduce((acc, key) => acc + Math.abs(userScores[key] - (constellation.stats[key] || 0)), 0)
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 pb-16 animate-in fade-in zoom-in-95 duration-500">
      ...
      <div className="grid md:grid-cols-5 gap-6 items-center mb-10">
        ...
        <div className="md:col-span-2 space-y-4">
          ...
          <p className="text-sm text-muted-foreground leading-relaxed">
            あなたの「惹かれる重要ポイント」と、{constellation.name}が持つ設計思想・役割パラメータを重ね合わせました。
          </p>
        </div>
      </div>
      ...
      {subcontractors.length > 0 && (
        <div className="mb-12">
          <h3 className="text-sm font-bold text-muted-foreground tracking-wider uppercase mb-4">あなたに近い他の星座候補</h3>
...
export default function ConstellationResultPage() {
  return (
    <Suspense fallback={
      <div className="max-w-3xl mx-auto py-24 flex flex-col items-center justify-center">
        <Loader2 className="w-10 h-10 animate-spin text-primary mb-4" />
